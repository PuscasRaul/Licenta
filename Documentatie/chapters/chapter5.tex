...
\begin{itemize}
  \item \textbf{Acuratețea globală:} 96\%,
  \item \textbf{Precizia medie:} 96\%,
  \item \textbf{Rata medie de regăsire (\textit{Macro Recall}):} 95\%,
  \item \textbf{Scorul mediu F1:} 96\%
\end{itemize}

Apropierea dintre valorile mediate macro și acuratețea globală, indică o performanță uniformă pe ansamblul claselor, fără simboluri puternic defavorizate. Acest comportament este așteptat, întrucât setul combinat menține un volum suficient de mostre pentru majoritatea claselor, evitând dezechilibrele. În ansamblu, o acuratețe de $96\%$ confirmă că reprezentarea liniarizată a imaginii, combinată cu nucleul RBF, captează suficientă informație discriminativă pentru separarea celor $36$ de clase.

Structura matricei de confuzie poate fi observată în figura~\ref{fig:matrice_confuzie} drept fiind puternic diagonală, erorile fiind rare și concentrate într-un număr restrâns de perechi de simboluri. Deși rare, erorile persistă în urma procesului de antrenare, în mod special la perechile de caractere care prezintă o similitudine a formei geometrice: pentru pereache formată din cifra \texttt{0} și litera \texttt{O} există 14 erori care apar în mod simetric. De asemenea, există confuzii pentru caracterele care sunt doar similare: litera \texttt{G} și \texttt{C}, cifra \texttt{1} și litera \texttt{I} sau chiar și litera \texttt{T}.

Aceste confuzii nu au, însă, o pondere uniformă în contextul aplicației. Confuziile de tip cifră--literă, precum \texttt{0}--\texttt{O} sau \texttt{1}--\texttt{I}, sunt în principiu recuperabile: poziția unui simbol în cadrul numărului de înmatriculare determină, conform formatului, dacă acesta trebuie să fie o cifră sau o literă, astfel încât o etapă ulterioară de decodare condiționată de format poate valorifica această cunoștință apriori pentru a le dezambiguiza. În schimb, confuziile de tip literă--literă (\texttt{G}--\texttt{C}, \texttt{M}--\texttt{H}) rămân nerezolvabile la nivel pur pozițional și constituie limita superioară a erorilor reziduale ale clasificatorului. Întrucât metrica end-to-end invalidează întregul rezultat la o singură eroare de caracter, reducerea acestor confuzii reziduale rămâne principala direcție de îmbunătățire a componentei de recunoaștere.
