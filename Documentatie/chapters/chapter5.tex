...
\indent\par
Antrenarea clasificatorului de caractere necesită un set de date format din imagini de caractere individuale, fiecare asociată cu eticheta sa. Construirea acestui set a urmat o strategie hibridă pe două surse: caractere generate automat prin aplicarea pipeline-ului de procesare asupra imaginilor de antrenare din setul Roboflow de plăcuțe, urmate de etichetare manuală, și un set extern de caractere individuale cu fonturi variate, adnotat în format Pascal VOC. Combinarea celor două surse urmărește, pe de o parte, alinierea distribuției datelor de antrenare la caracteristicile particulare ale caracterelor segmentate de pipeline-ul propriu și, pe de altă parte, asigurarea unui volum și a unei diversități tipografice suficiente.

\subsubsection{Generarea automată prin fluxul de procesare}
\indent\par
Prima sursă de date este generată de însuși sistemul, prin scriptul \texttt{generate\_dataset.py}. Acesta aplică, asupra imaginilor de antrenare din setul Roboflow de plăcuțe, etapele de localizare și de segmentare descrise anterior, salvând caracterele segmentate drept imagini individuale, încă neetichetate. Avantajul acestei surse este că mostrele rezultate reflectă fidel caracteristicile caracterelor așa cum sunt produse de pipeline-ul propriu — inclusiv eventualele artefacte de segmentare —, ceea ce aliniază distribuția datelor de antrenare la cea a datelor întâlnite la rulare. Mostrele astfel obținute necesită o etapă ulterioară de etichetare.

\subsubsection{Sursă externă de caractere}
\indent\par
A doua sursă de date o constituie un set de date de caractere individuale, disponibil prin platforma Roboflow, care acoperă o varietate mai largă de fonturi față de cele prezente în setul de plăcuțe. Adnotările sunt furnizate în format Pascal VOC (fișiere XML). Scriptul \texttt{parse\_characters.py} parcurge perechile imagine–adnotare, extrage din fiecare fișier XML etichetele caracterelor și dreptunghiurile lor de încadrare, decupează regiunile corespunzătoare din imagine și le salvează în directoare organizate pe clase (câte un director per caracter). Rezultatul este un set de mostre curat etichetate, obținut fără efort manual, care completează diversitatea tipografică a primei surse.

\subsubsection{Etichetarea manuală asistată}
\indent\par
...
\subsubsection{Filtrarea și igienizarea claselor}
\indent\par
